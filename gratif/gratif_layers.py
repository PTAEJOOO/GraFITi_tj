...
class Encoder(nn.Module):
    ...
    def forward(self, context_x, value, mask, target_value, target_mask):
        ...
        target_mask_ = torch.stack([pad(r[m]) for r, m in zip(target_mask, mk_bool)]).contiguous()  # BxK_ = Bxfull_len // target_mask에서 observation mask==1인 값들만 가져옴. 나중에 output masking 할때 사용
