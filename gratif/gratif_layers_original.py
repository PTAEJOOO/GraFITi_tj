...
class Encoder(nn.Module):
    ...
    def forward(self, context_x, value, mask, target_value, target_mask):
        ...
        U_ = torch.stack([pad(r[m]) for r, m in zip(value, mk_bool)]).contiguous()  # BxTxC (values) -> Bxfull_len // observation의 edge indicator
        target_U_ = torch.stack([pad(r[m]) for r, m in zip(target_value, mk_bool)]).contiguous()  # BxK_ = Bxfull_len
        target_mask_ = torch.stack([pad(r[m]) for r, m in zip(target_mask, mk_bool)]).contiguous()  # BxK_ = Bxfull_len // target_mask에서 observation mask==1인 값들만 가져옴.
        ...
        print(f"edge embedding size = {U_.size()}, time node embeeding size = {T_.size()}, channel node embedding size = {C_.size()}")
        ...
        for i in range(self.n_layers):
            ...
            ## 'T_inds_' 가 정의된 걸 보면, timepoints index인 'T_inds'에서 mk_bool==1을 만족하는 index만 가져오니까, 각 channel이랑 연결된 timepoints만 얻을 수 있음.
